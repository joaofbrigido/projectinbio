...
export async function POST(req: NextRequest) {
  try {
    ...
    switch (event.type) {
      case "checkout.session.completed": // Usuario completou o checkout - assinatura ou pagamento unico
        ...
        break;
      case "checkout.session.async_payment_succeeded": // Usuário pagou o boleto
        if (event.data.object.payment_status === "paid") {
          const userId = event.data.object.client_reference_id;
        ...
        break;
      case "customer.subscription.deleted": // usuário cancelou assinatura
